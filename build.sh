...
Commands:
  ...
  usb <dev>   Hand the rest over to ./usb.sh, which builds usb.img from lfs/
              and pours it onto a USB disk with dd. Erases the device, which
              must be named in full. USB_YES=1 skips the typed confirmation.
              The root filesystem is then grown to fill the stick. Needs root
...
Notes:
  ...
  virt, usb and chroot need root: they use losetup, mount, dd and chroot.
  usb is ./usb.sh, which is forwarded to and can be run on its own.
...
# usb lives in usb.sh now, and that script can also be run directly.
#
# Forwarded rather than dropped: "./build.sh usb" is what the README documents
# and what is in the shell history, and its two arguments -- a device, or
# "image" -- pass straight through. exec, so the exit status and the signals
# are usb.sh's own; the traps that clean up after an interrupted write are set
# over there and would be no use here.
if [ "$1" == "usb" ]; then
  shift
  exec "${working_directory}/usb.sh" "$@"
fi
